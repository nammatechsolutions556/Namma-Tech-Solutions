...
exports.getDashboardStats = async (req, res) => {
    try {
        // Run all count and data queries in parallel
        const [
            projectsCountResult,
            ...
            pool.query('SELECT COUNT(*) FROM project_requests'),
            pool.query('SELECT COUNT(*) FROM certificates'),
            pool.query('SELECT COUNT(*) FROM contact_messages'),
            pool.query('SELECT * FROM project_requests ORDER BY created_at DESC LIMIT 5'),
